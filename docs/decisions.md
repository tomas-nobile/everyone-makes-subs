...
- 2026-09-24 · DECISION: no GitHub Actions. `release.yml` removed; F01.4 is done as the Electron hello world + a local `npm run app:dist` (Windows NSIS verified). F14.3 (installers on every tag) is cut; installers are built by hand per OS.
