...
| `npm run clip -- <youtube-url> --from=03:00 --to=04:30 --lang=en` | A clip of a real talk with subtitles generated by the system, burned in |
| `npm run demo-video [-- --replay]` | Records the demo video from the running system (Electron + ffmpeg): title cards, the subtitled clip, dashboard, phones, TV, overlay |
| `npm run load` | 2,000 SSE viewers on one stage |
